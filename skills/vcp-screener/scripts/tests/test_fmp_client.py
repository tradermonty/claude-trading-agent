...
class TestFMPClientInit:
    ...
    def test_uses_explicit_api_key_and_sets_header(self, mock_session):
        client = FMPClient(api_key="dummy-key")  # pragma: allowlist secret

        assert client.api_key == "dummy-key"
        assert mock_session.headers["apikey"] == "dummy-key"

    def test_uses_environment_api_key(self, monkeypatch, mock_session):
        monkeypatch.setenv("FMP_API_KEY", "env-dummy-key")  # pragma: allowlist secret

        client = FMPClient()

        assert client.api_key == "env-dummy-key"
        assert mock_session.headers["apikey"] == "env-dummy-key"
