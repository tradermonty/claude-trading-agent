...
def validate_result(result: dict) -> tuple[bool, list[str]]:
    ...
    # Determine validity from required fields only
    ...
    if state == "Breakout":
        for field in BREAKOUT_EXTRA_FIELDS:
            if _get_nested(result, field) is None:
                warnings.append(f"missing Breakout field: {field} (non-fatal)")
...
def _build_actionable(
    ...
    if valid_date.weekday() >= 5:  # Saturday or Sunday
        valid_date += timedelta(days=7 - valid_date.weekday())
