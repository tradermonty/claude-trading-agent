...
> **費用に関する注意**: このアプリは Anthropic API の**従量課金**を使用します。チャットやスキル実行のたびにトークン課金が発生します。目安：
>
> | 操作 | 推定コスト |
> |------|-----------|
> | 通常のチャット1回 | ~$0.01〜0.05 |
> | スキル1回実行（例: `/ftd-detector`） | ~$0.10〜0.50 |
> | フルワークフロー（5スキル以上連続） | ~$1.00〜3.00 |
>
> コストはモデル（デフォルト: `claude-sonnet-4-6`）、プロンプトサイズ、エージェントのコード実行量に依存します。[console.anthropic.com](https://console.anthropic.com/) で使用量を確認し、請求アラートを設定することを推奨します。
